...
static const AptFlavor apt_flavors[] = {
    { "PCLinuxOS Debian", "pclinuxos",       "/etc/pclinuxos_version",       "/etc/os-release",            "VERSION_ID="          },
    { "PCLinuxOS",        "pclinuxos",       "/etc/pclinuxos_version",       "/etc/os-release",            "VERSION_ID="          },
...
static void get_zswap_info(OperatingSystem *os)
{
    ...
    os->zswap.shrinker_enabled = val && g_str_equal(val, "Y");
    g_free(val);

    os->zswap.compressor =
        h_sysfs_read_string("/sys/module/zswap/parameters", "compressor");
    if (!os->zswap.compressor)
        os->zswap.compressor = g_strdup(_("Unknown"));
...
gchar *computer_get_landlock_abi_version(void) {
  ...
  if (abi <= 0)
    return g_strdup(_("Unknown"));
  return g_strdup_printf(_("Version %d"), abi);
}
#endif
